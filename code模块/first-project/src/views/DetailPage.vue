...
export default {
    ...
    methods: {
        loadDetailById() {
            this.$api.loadFilmsDeatil(this.params)
        ...
        loadCommoneById() {
            this.$api.loadFilmsCommone(this.params)
            .then(( { resultState,result,msg } ) => {
                if(!resultState){
                    return Promise.reject(msg);
                }
    ...
    beforeMount(){
        this.params.filmId = this.$route.query.filmId;
        this.$root.$on("flash-comm",this.loadCommoneById);
    },
    mounted() {
        this.loadDetailById();
